...
class SBL_GRW():
    """
    DESCRIPTION: Gaussian Random Walk (GRW) Bayesian Sequential Learning Agent
        * Agent parses a binary sequence previously generated by HHMM
        * She updates her conjugate G posterior with new evidence
    ...
    """
    def __init__(self, seq, sigma):
        # Initialize SBL-learned sequence and hidden state variance param
        self.sequence = seq[:, 1]
        self.hidden = seq[:, 0]
        self.T = len(seq)
        self.sigma = sigma
...
def main(prob_regime_init, prob_regime_change,
    ...
    GRW_SBL_temp = SBL_GRW(hhmm_seq, sigma)
    results = GRW_SBL_temp.compute_surprisal(model)

    if save_results:
        title = "sbl_grw_surprise_" + str(model) + "_" + str(seq_length) + ".txt"
